...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @objc func verileriAl() {
        ...
        fetchRequest.returnsObjectsAsFaults = false //ozellikle buyuk verilerle calısırken
         
        do {
            let sonuclar = try context.fetch(fetchRequest)
            if sonuclar.count > 0 {
                 
                for sonuc in sonuclar as! [NSManagedObject] //any tipinde oldugu icin nsmanagedobject olarak cast edilir(coredatatan gelecegı için veri)
                {
